    ...
    void setAlarms(String key, String startTime, String stopTime, Context ctx) {
        ...
        if (ctx != null) {
            ...
            if (alarmMgrStart != null) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && !alarmMgrStart.canScheduleExactAlarms()) {
                    // The app doesn't have the permission to schedule exact alarms.
                    // You can request the permission by redirecting the user to the app's system settings,
                    // or handle this situation according to your app's requirements.
                    Intent intent = new Intent("android.app.action.REQUEST_SCHEDULE_EXACT_ALARM");
                    intent.setData(Uri.parse("package:" + ctx.getPackageName()));
                    ctx.startActivity(intent);
                } else {
                    // The app can schedule exact alarms, proceed with scheduling the alarm.
